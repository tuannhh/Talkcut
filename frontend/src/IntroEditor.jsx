...
export function IntroEditor({clip,setting,setPreview,suggestIntro,busy,assets,refresh,post,notify,media,Field,Slider,Toggle,AssetPicker,onUpload,onImageDrag}){
 ...
 return <div className="intro-editor">
  ...
  <Toggle label="Ảnh nền thương hiệu phía trên" checked={s.intro_background_enabled} onChange={v=>setting('intro_background_enabled',v)}/>
  {s.intro_background_enabled&&<><AssetPicker label="Ảnh nền intro" kind="image" value={s.intro_asset} assets={assets} onChange={v=>setting('intro_asset',v)} onUpload={()=>onUpload('intro_asset')}/><Slider label="Chiều cao vùng ảnh nền" min={.1} max={.6} value={s.intro_background_height} onChange={v=>setting('intro_background_height',v)}/></>}
  ...
  <AssetPicker label="Hoặc tải ảnh intro của bạn" kind="image" value={s.intro_image_asset} assets={assets} onChange={v=>{setting('intro_image_asset',v);setPreview('intro');}} onUpload={()=>onUpload('intro_image_asset')}/>
  ...
  <Slider label="Kích thước tiêu đề" min={28} max={110} step={2} value={s.intro_title_size} format={v=>v+' px'} onChange={v=>setting('intro_title_size',v)}/>
  ...
  {s.intro_tts?<><div className="voice-mode-options" role="radiogroup" aria-label="Cách tạo giọng đọc">{[['prebuilt','Sử dụng voice có sẵn của Google'],['designed','Tạo sinh giọng đọc theo tùy chọn']].map(([v,label])=><label key={v}><input type="radio" name="voice-mode" checked={s.intro_voice_mode===v} onChange={()=>setting('intro_voice_mode',v)}/><span>{label}</span></label>)}</div>
   ...
   <Toggle label="Hiển thị phụ đề karaoke ở Intro" checked={s.intro_caption_enabled} onChange={v=>setting('intro_caption_enabled',v)}/><Slider label="Vị trí phụ đề intro" min={.3} max={.83} value={s.intro_caption_y} onChange={v=>setting('intro_caption_y',v)}/><p className="helper">Karaoke dùng kiểu chữ trong tab Phụ đề. Lời dẫn không còn hiển thị thành một khối chữ tĩnh.</p><VoiceControls settings={s} onChange={setting} post={post} notify={notify} media={media}/>
