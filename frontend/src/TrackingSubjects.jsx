...
export function TrackingSubjects({clip,api,media,onSelect,jobs,disabled=false}){
 ...
 return <div className="tracking-subjects"><h3>Chọn chủ thể để AI theo dõi</h3><p className="helper">AI tự tìm các khuôn mặt rõ trong chính đoạn clip này. Chọn một khuôn mặt, AI sẽ tự bắt đầu theo dõi qua các góc máy.</p>
  ...
  <div className="subject-gallery">{items.map((s,i)=><button key={s.id} disabled={working||disabled} className={clip.settings.tracking_subject===s.id?'selected':''} aria-label={'Chọn khuôn mặt '+(i+1)} onClick={()=>onSelect(s.id)}><img src={media(s.path)} alt={'Khuôn mặt gợi ý '+(i+1)}/><span>{(s.time-clip.start).toFixed(0)}s</span></button>)}</div>
  {items.length>0&&!working&&<button className="text-action" onClick={()=>scan(true)}>Gợi ý lại khuôn mặt</button>}
  {clip.settings.tracking_subject&&<><p className="helper">Đã chọn chủ thể. AI sẽ giữ đúng người này khi chuyển góc máy; cảnh chưa đủ chắc chắn vẫn dùng video đang chuyển động và được đánh dấu cần duyệt.</p><button disabled={disabled} className="secondary small" onClick={()=>onSelect(null)}>Bỏ chọn chủ thể</button></>}
 </div>;
}
