...
def test_presets_persist_styles_without_replacing_clip_content():
    ...
    origin=Settings(intro_text='Lời dẫn clip cũ',intro_title_text='Tiêu đề cũ',intro_title_highlight='cũ',intro_image_asset='old-photo',intro_frame_time=55,caption_size=80,transition_seconds=.9).model_dump()
    ...
    assert applied['caption_size']==80 and applied['transition_seconds']==.9
...
def test_saved_v4_migrates_once_and_new_mix_can_be_disabled():
    assert Settings.model_validate({'crop_mode':'auto','transition_seconds':.24}).transition_seconds==.65
    assert Settings.model_validate({'crop_mode':'auto','transition_seconds':0,'pacing_version':5}).transition_seconds==0
    assert Settings(transition_seconds=1.2).transition_seconds==1.2
